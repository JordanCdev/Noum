...
- OK — The real `recommendation set` generator that initiative #1 wired the verdict into is `NextActionEngine.recommend`, and it is time-independent (snapshot-safe)
    _Noum/NextActionEngine.swift:131 `static func recommend(input: NextActionInput) -> NextAction`; NextActionInput is a flat value struct (:89-114) with recommendationOutcomes already defaulted (:113). grep for Date() in NextActionEngine returns ONLY LastNextActionSnapshot.save/load (:68,:81) — recommend() and its tiers touch no clock. NextAction (:7-12) + ActionRecommendation (:15-50, Equatable by displayTitle) are snapshot-projectable._
