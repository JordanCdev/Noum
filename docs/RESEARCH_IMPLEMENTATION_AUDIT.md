...
`77b1361a965acd0d0703ef873b16a840c468da66`
...
inventory, minimum-client decision, emulator execution, or two-device
...
The last complete clean-source regression passed 3,946 tests across 411 suites with
zero failures or skips, and its unsigned optimized Release simulator build
succeeded at `47cbab5f`. Xcode logged one transient parallel-clone launch
rejection, retried the work, and produced a Passed result containing the
complete count. The canonical coach artifact passes app-path, real-pipeline,
and trace-quality gates for its named `80fbf6d2` boundary, but the hardened
preflight now rejects it as current-source evidence because committed descendant
changes include behavior-bearing paths outside the old manual fingerprint. The
...
implementation is `77b1361a`. It remains valid historical evidence for that
...
completed full regression proves only its recorded `47cbab5f` source boundary.
...
| Current full integrated Swift attempt | **Incomplete** | At clean implementation commit `d1d25ca6`, `/tmp/NoumExactDemandFull.xcresult` recorded 4,032 passing tests, 18 failures, and zero skips. Failures clustered around parallel account-bootstrap/onboarding UI flows, recommendation/phrase routes affected by that bootstrap state, one missing simulator clone, and one isolated Ask Noum diagnostic assertion. This is failed evidence, not a current green regression, and it does not supersede `47cbab5f`. |
...
| Exact executed practice demand | **Proved** | At detached clean `ea7a3845`, Functions lint and all 67 Node tests passed. A fresh iPhone 17 Pro simulator build passed 43/43 focused tests across `PracticeSessionDemandTests`, `GoalOutcomeLoopTests`, and `AccountDataRegistryTests`; result bundle `/private/tmp/NoumExactDemand-ea7a3845.xcresult`. They prove current/legacy codecs, exact Timed/Pressure/Project comparison, comparison-schema v2 fail-closed behavior, intent-copy retention, and account-isolated export/deletion. Source rules also prove the current private-session allowlist includes metric provenance and mode-coupled demand; the authored emulator allow/reject matrix was not executed because the local Java/supported-Node prerequisites remain absent. The light screenshot sweep was blocked by the simulator account-bootstrap recovery surface and does not visually prove the share-card label. |
| Exact prescribed Timed demand | **Proved** | At implementation commit `d1d25ca6`, `/tmp/NoumExactDemandTests.xcresult` passed 89/89 selected simulator tests with zero failures or skips across exact demand persistence, route projection, tap attribution, reconciliation, availability, and legacy fail-closed behavior. At final detached clean source boundary `77b1361a`, Functions lint and all 69 Node tests passed and the unsigned optimized Release simulator build succeeded. `/tmp/NoumExactDemandRouteUI.xcresult` also passed the serial seeded Train contract: it rendered `Recommended difficulty, Medium`, entered the existing Timed prompt, and retained a destination attachment. The light sweep's five ordinary tabs were blocked by account bootstrap; `.screenshots/2026-07-14_prescribed-timed-demand/02_train_seeded_beginner.png` separately verifies the restrained demand capsule. These local contracts do not prove a deployed cross-device ledger, normal-account five-tab routing, physical-device behavior, or population effectiveness. |
| Recommendation sync ordering, hydration, and local CAS source | **Proved** | At detached clean source commit `9378b6ef`, `/private/tmp/NoumRecommendationSyncCommit9378b6ef.xcresult` passed 81 selected simulator tests with zero failures or skips across first-run hydration, recommendation sync, goal outcomes, release identity/privacy, and account-data registry coverage. At detached clean `00dd5975`, Functions lint and 66/66 Node tests passed; `/private/tmp/NoumRecommendationCAS-00dd5975.xcresult` passed 28 focused iPhone 17 Pro simulator tests with zero failures or skips. The newer contracts cover strict legacy/versioned envelopes, expected-revision mutation, exact-body idempotent replay, Unix wire dates, crash-safe persistence ordering, bounded one-shot conflict rebase, direct-write denial, account-scoped cursor/mutation ownership, and fail-closed REST writes. Emulator source additionally covers a concurrent same-revision race, migration/replay/conflict, deletion tombstone, and direct-write denial, but was not executed because this machine lacks Java and uses unsupported Node 26. This proves named local source contracts, not deployed Firebase behavior, runtime IAM/App Check, mixed-client compatibility, or two-device conflict recovery. |
| Coach-arena contracts at the evidence baseline | **Proved** | 119 Node contracts and 136 Python runner contracts pass. The suite proves default-deny committed and uncommitted source classification, NUL-safe rename/copy/untracked handling, exact generated-output exclusions, no-overwrite sidecar refusal, and existing score/trace/readiness behavior. The canonical artifact remains bound to `80fbf6d2`; it is no longer current for `77b1361a`. These are local tooling and evaluation contracts, not live-provider or human-outcome proof. |
...
| Production readiness | **Missing** | The clean detached `77b1361a` readiness rerun remains NO-GO, 18/100, local target shape 85/100, maximum allowed 20/100, claim `localEvaluationSubstrateOnly`. Static operational wiring passes 19/19, but source freshness blocks because the active dump/report remains bound to `80fbf6d2`, not `77b1361a`. No validated attachment-backed release run is available, one of five required sidecars is present, and zero of five passes. The main checkout additionally preserves the user's unrelated localization edit. |
...
as named historical baselines; neither is the current `77b1361a` source
boundary. `9378b6ef` remains the process-local recommendation sync boundary,
`eb597589` remains the tap-attribution boundary, and `00dd5975` is the local CAS
source boundary. `ea7a3845` is the exact executed practice-demand boundary;
none replaces the complete `47cbab5f` regression. Regenerate the local
...
readiness past the external cap by themselves. The current primary checkout is
intentionally not eligible for refresh while the user's localization edit is
uncommitted. The Home/Train tap-time denominator repair is complete at
`eb597589`, and exact executed Timed, Pressure Drill, and Speech Project demand
is persisted at `ea7a3845`. Exact prescribed Timed demand is carried and
fail-closed at `d1d25ca6`, with its serial rendered route assertion at
`77b1361a`; that local coaching gap is no longer next work. The highest-leverage
safe local repair is now to isolate the account-bootstrap and parallel-clone
failures from the current full scheme attempt, restore a clean complete
regression at the current boundary, and refresh the stale local app-path
artifact only from a clean checkout. Local recommendation CAS source is present
at `00dd5975`; the remaining distributed integrity work is operational.
Provision and verify the dedicated runtime identity, execute the emulator suite
on the supported Java/Node toolchain, inventory active clients, approve a
minimum-client policy, and stage the callable before the coordinated
social/rules cutover. A two-device mixed-build conflict/replay/deletion smoke,
plus backup and rollback evidence, must accompany that deployment.
