...
| Full callable runtime-security inventory | **Proved locally** | At `81c17f4c`, the verifier requires the exact 13 exported callables, including both competitive-observation endpoints, their five reviewed dedicated identities, exact production project/region, App Check and trusted-caller source contracts, least-privilege role boundaries, and unauthenticated `401` probes for the same roster. Its local contract and source checks pass. No trusted authenticated production inspection was run, so deployed IAM, App Check, function inventory, and endpoint behavior remain externally unproved. |
| Source-exact hosted privacy enforcement | **Proved locally** | At `518b3103`, shell and Python readiness paths share bounded decompression and require the approved origin, 2xx HTML, and exact `public/privacy.html` bytes without logging body content. The focused privacy contracts pass. Processor manifest v4 changes the current body at `81c17f4c` to 27,569 bytes / `9ee5fb73…4dc7e5`; the last live probe targeted an older source body, so hosted equivalence remains Missing until an authorized deployment and fresh current-source probe pass. |
...
| Ineligible competitive observation substrate | **Proved locally** | Commits `32ef79a8`, `e2243e3d`, `76590f2d`, and `81c17f4c` add exact Auth/App-Check/cutover-gated begin/complete contracts, bounded canonical PCM, an at-most-once provider claim, transcript-free permanently ineligible receipts, direct-rule denial, account deletion, processor-manifest v4 disclosure, and a false-gated iOS Ah Counter route that reuses the existing microphone tap and private fallback. Functions lint/build, 100/100 unit tests, 6/6 deploy-blocker tests, a clean detached Node-22/Java-21 `demo-noum` run with 25/25 callable/rules plus 6/6 real Firestore-adapter tests, and an integrated 55/55 iPhone 17 simulator selection pass. This proves local observation mechanics only. Non-challenge provenance remains client-asserted, replay protection is per account, expiry has no externally proved TTL, and there is no calibrated evaluator or eligible evidence write. |
...
| Trusted competitive session-evidence producer/evaluator | **Missing for eligibility; observation substrate proved locally** | `_verifiedSessionEvidence` has a strict server consumer contract, and the new disabled path can server-observe bounded PCM through Deepgram and persist only a transcript-free, permanently ineligible receipt. It deliberately never writes eligible evidence. Non-challenge mode/demand/prompt provenance remains client-asserted, replay protection is per account, retention TTL is unproved, and no deterministic evaluator, calibration artifact, evidence-floor policy, or production authorization exists. Client sessions, local scores, observation receipts, and emulator fixtures cannot close this row. |
| Reciprocal friendship authority | **Missing** | Callables require exact reciprocal server-link schemas, and cutover v3 quarantines every legacy friend row instead of promoting it. There is still no server-owned request/accept/remove lifecycle that can legitimately materialize new reciprocal links. Challenges and friend-scoped reads cannot be production-authoritative until that lifecycle exists. |
...
| Hosted Firebase privacy body matches source | **Missing** | Current source is 27,569 bytes / `9ee5fb73…4dc7e5`. The last live probe targeted an older source body, and no `81c17f4c` exact-body probe exists. An authorized deploy and a fresh passing probe are required. |
...
closing cross-account replay and retention proof, plus a server-owned reciprocal
friend lifecycle.
