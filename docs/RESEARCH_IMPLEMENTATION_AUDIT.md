...
| Real app transport coverage in live-provider evidence | **Missing** | Existing app-path reports inject authored replies, while provider sweeps do not prove signed app authentication, App Check, composer dispatch, callable completion, and final visible copy as one route. Focused entitled-simulator tests prove local wire, decode, intent, provenance, evidence, and landing contracts with an injected transport only. Required evidence must bind app SHA, signing/entitlement state, Firebase identity type, App Check state, deployed function revision, policy version, generation mode, model, and actual user-visible response. |
...
| Full callable runtime-security inventory | **Incomplete** | Current source requires exactly 18 reviewed callable exports plus one reviewed scheduled function, 19 Functions exports total; the static cloud-operations validator passes 19/19. A read-only production query returned only four active functions—`coachChat`, `coachChatAvailability`, `deleteAccount`, and `transcriptionToken`—and therefore proves that the app-required v2 route is absent. The v1 behavior freeze is local semantic-contract proof only. Backend-first closure must deploy and read back both the additive `coachChatV2` route and the updated availability contract; authenticated mixed v1/v2 smoke, adoption measurement, and rollback proof are unperformed. Deployed IAM, App Check, indexes, and endpoint behavior remain externally unproved. |
