...
Committed implementation and the scoped Ask Noum deployment were inspected
through `08985b393`. Earlier rows retain their source-bound historical evidence;
the newest deployment does not promote local or historical results into
...
On 2026-07-19, the authorized scoped release deployed this exact Functions
source from commit `08985b39321095edec52c48ca50317763454d736`, whose tracked
Functions digest is
`20949ca38bd48d9cb164d8114e3d69c51fc5860b83b5d29cacb8405f5f542137`.
The selector contained only `coachChatV2` and `coachChatAvailability`.
Predeploy lint, **162/162** Functions tests, **9/9** release-lock tests, and the
static 18-callable/1-schedule operations validator passed. Both functions read
back `ACTIVE`, Node 22, and all traffic on revisions
`coachchatv2-00002-xiq` / `coachchatavailability-00004-jek`, using the dedicated
`noum-coach-runtime@noum-d0b6f.iam.gserviceaccount.com` identity. Both callable
transport bindings are present; unauthenticated direct probes reach the
handlers and return HTTP 401 `UNAUTHENTICATED` as required. A newly signed
Debug simulator build installs and launches, but its account state is at fresh
onboarding, so the exact conversation has not been retried and the five-tab
sweep still does not exercise the Ask Noum conversation surface.
...
A later installed-simulator turn isolates the next failure after deployment.
At 22:02 UTC production completed two v2 calls with HTTP 200, Firebase Auth and
App Check `VALID`, verified account binding, and `gemini-2.5-flash` generation;
the substantive response contained 33 output tokens. The exact simulator log
then recorded `failure=contentRejected`. The client had discarded a completed
server answer because its general-coaching landing gate required an observable
rep-style anchor on every non-greeting, while Functions requires the concrete
action/bridge shape only when the user explicitly asks for a move. The old
`unauthenticated` console line and simulator network/audio warnings were not the
cause of this turn.

Current dirty source aligns that client rule with the server: a statement-
shaped general-coaching disclosure may receive a concise explanation after the
common safety, overclaim, length, repetition, and style gates pass, while an
explicit `how do/can/could/should/would I` request still requires a concrete
move, communication anchor, and rationale. The existing reliability gate now
has two bounded wit/on-the-spot recovery variants, and secure diagnostics retain
the stable rejected-gate label without user or draft text. The exact regression
failed before the edit and passes 3/3 afterward. The broader reply-quality,
wire-routing, and reliability-gate selection passes 331/331 with zero failures
or skips. A current Debug build was installed over the existing
iPhone 17 simulator app with the composer enabled. No post-install live turn was
submitted, so this is local deterministic repair and installation evidence—not
generated wording acceptance, reporter re-acceptance, professional review, or
an external release artifact.
...
empty or partial case coverage, fewer than three distinct professional reviewers
...
| Compute a public 0–100 style score for every session | **Contradicted** | The product deliberately exposes `GoalOutcomeRead` as qualitative evidence/movement. `GoalStyleCalibrationCandidate` can produce a versioned 0–100 candidate only for access-controlled professional calibration and is explicitly disconnected from product views, stores, analytics, sessions, and export. Its result gate requires complete three-reviewer professional coverage, exact rubric shape, and reviewer-bound evidence-access receipts; that integrity contract is not a calibration result. |
...
| Ask Noum v2 production capability readback (2026-07-17 and 2026-07-19) | **Partially proved live** | Source-bound commit `86334ed45` added the checked-in two-function release path and first deployed `coachChatV2` plus `coachChatAvailability`. On 2026-07-19, the same exact selector deployed the current continuity/quality/diagnostic Functions source from `08985b393` with tracked digest `20949ca3…f542137`. Predeploy lint, 162/162 Functions tests, 9/9 release-lock tests, and the 18-callable/1-schedule operations validator passed. Both functions read back `ACTIVE`, Node 22, all traffic on revisions `coachchatv2-00002-xiq` / `coachchatavailability-00004-jek`, and the dedicated `noum-coach-runtime` identity. Callable invoker bindings remain present while unauthenticated direct probes reach the handler and fail closed with HTTP 401. The earlier authenticated simulator greeting proved one valid trivial generation; the current deployment has no authenticated generated turn because the reinstalled simulator is at fresh onboarding. This proves delivery and admission boundaries, not acceptable coaching quality, mixed-client behavior, rollback, independent review, or production readiness. |
...
| Live generated reply quality | **Missing** | The reporter's real acceptance failed: replies were described as weird, redundant, and unlike a human expert communication coach. Production later generated a 33-token substantive response with HTTP 200 and valid Auth/App Check, but the installed client discarded it as `contentRejected` because its local general-coaching rule was stricter than the server contract. Current dirty source aligns statement-shaped turns with the server, preserves strict explicit-action grounding, adds bounded wit/on-the-spot recovery, passes its exact 3/3 regression, and passes the broader reply-quality/wire/reliability selection 331/331. That build is installed on the simulator, but no post-install live turn, representative conversation set, reporter re-acceptance, or independent professional review exists. |
| Real app transport coverage in live-provider evidence | **Incomplete** | Installed iPhone 17 simulator evidence proves composer dispatch, valid Auth/App Check, verified account binding, v2 callable completion, one visible greeting, and a later 33-token substantive generation. The latter did not render because the previous client rejected it locally. The repaired current build is installed with account/chat state preserved and the composer enabled, but it has not submitted a post-install live turn and no capture binds an exact app commit/signing artifact into a managed release attachment. Required evidence still needs representative visible responses tied to app SHA, signing/entitlements, Firebase identity, and deployed revision. |
...
| Source-exact public-web enforcement | **Proved locally** | Shell, App Store, cloud-operations, and Python-readiness paths use one 256 KiB verifier and require an approved direct origin, HTTP 200 HTML, and exact homepage/privacy/support/coaching bytes without logging content. Firebase remains the active app/metadata origin while `noum.app` is parked; a dual-origin gate must pass before source cutover. Hosted equivalence remains Missing until an authorized deploy and fresh current-source probe pass. |
...
| Production readiness | **Missing** | The current continuity/quality/diagnostic Functions source is deployed from `08985b393`, both exact functions are active on their latest revisions, and unauthenticated admission fails closed. The bounded iOS selection passes 293/293, Functions pass 162/162 plus 9/9 release-lock checks, and a signed Debug build launches, but seven evidence-provenance fallback fixtures remain explicitly open and the current Ask Noum conversation was not authenticated, visually exercised, or live-provider accepted after deployment. This source has not completed the reporter's exact live retry, a representative generated-output sweep, mixed-client/adoption/rollback proof, signed full-scheme/UI/VoiceOver/physical-device verification, reporter acceptance, or independent professional review. Wider content-sync convergence, schema-2 production rules, and complete 19-export runtime inventory also remain open. The authoritative result therefore remains **NO-GO at 18/100 with 0/5 required external artifacts**; deployment earns no readiness points by itself. |
...
| Longitudinal real-user transfer outcomes | **Missing** | `coach-real-user-transfer-outcomes-v4.json` is absent. Local fixtures cannot earn this row. The release row requires 28 elapsed days, 30 qualified qualitative participants, and 200 qualified D1/D7 installs; the distinct 500-install scale signal is nonblocking. |
| Physical-device TestFlight QA | **Missing** | `coach-real-device-testflight-qa-v3.json` is absent. Its fail-closed contract requires exactly 14 named surfaces and 84 named checks from the same independently verified physical TestFlight build; simulator and direct development-device builds cannot count. |
