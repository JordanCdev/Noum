...
| Current-source unit regression | **Proved locally** | At committed source `ac664112`, the serial `NoumTests` target records 4,109/4,109 tests, zero failures, zero skips, and result `Passed` on the iPhone 17 simulator. The focused coach-filler/context/provider/conversation selection separately passes 227/227. This does not supersede the clean signed full-scheme, Release-scan, UI, physical-device, deployed-backend, or external evidence gates. |
