...
| Goal read → auto-prescribed drill | **Incomplete** | `NextActionEngine` owns the action, but it does not consume `GoalOutcomeRead.nextDimension`, movement, or its qualitative proof test. The explicitly chosen style currently affects rationale copy and a late/default drill tie-breaker; Summary computes the outcome read separately after the finalized action exists. |
...
| Severe filler destination and shared recommendation-rate policy | **Proved locally** | At `c3d1ff65`, one pure `FillerBurden` projection drives severity, drill focus, cross-session trends, filler-aligned copy, and controlled pressure-stretch eligibility. At `ec10990f`, severe qualifying burden routes to Ah Counter (`Filler Control`), including after a first severe qualifying rep; non-severe thin evidence stays suppressed. Home/Train recommendation inputs and Summary's shared builder use qualifying per-session rates and exclude sub-15-second samples. Boundary tests cover 10/600, 3/20, exact 8/min, count/duration floors, simultaneous severe filler/pace, explicit target authority, first-rep behavior, invalid values, and normalized downstream response/adaptation behavior. Focused tests pass 93/93 and the full simulator unit target passes 4,065 tests with zero failures or skips. Remaining raw-count descriptive comparisons in `CoachingPlanner`, Summary comparison copy, and Path quality are outside this adaptive-prescription proof and still need review before making duration-sensitive claims. This proves local routing consistency, not professional calibration or user benefit. |
| Production readiness | **Missing** | The clean detached `70b0b380` readiness rerun remains NO-GO, 18/100, local target shape 85/100, maximum allowed 20/100, claim `localEvaluationSubstrateOnly`. Descendant commits through `ec10990f` add local ineligible observation, friendship authority, bounded cross-account exact-PCM replay, goal-calibration review-intake integrity, dormant exact challenge provenance, a process-local exact-session route lease, checked-in Firebase backend deployment locks, duration-normalized filler prescription, and the literal severe-filler-to-Filler-Control route, but earn no external readiness points. The current Functions source has a clean canonical local gate, not a new attachment-backed release capture. No validated attachment-backed release run is available, zero of five required external sidecars is present, and no current-source hosted privacy-body probe exists. The main checkout additionally preserves the user's unrelated localization edit and staged screenshot handoffs. |
