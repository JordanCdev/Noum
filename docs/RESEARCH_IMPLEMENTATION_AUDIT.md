...
| Goal read → auto-prescribed drill | **Incomplete** | `NextActionEngine` owns the action, but it does not consume `GoalOutcomeRead.nextDimension`, movement, or its qualitative proof test. The explicitly chosen style currently affects rationale copy and a late/default drill tie-breaker; Summary computes the outcome read separately after the finalized action exists. At `ac393554`, the goal/trajectory evidence itself becomes quantity-qualified and duration-normalized. At `dbafb01b`, Summary and `CoachingPlanner` share the same qualified filler-rate boundary. Routing remains intentionally deferred until the corpus-bound direct coach-context filler comparison also shares that boundary and goal weighting can be added without displacing blocker, severe, or case evidence. |
...
| Graceful failover banner | **Incomplete** | Requested-cloud/resolved-local startup produces one content-free, accessibility-labelled notice on Timed, Pressure, Ah Counter, IM, Mini-drills, and Lesson Apply. Deliberate local use and successful cloud stay quiet. Pace Training, Cut the Crutch, and Roleplay still need the same modifier before this can be claimed across every speech surface. |
...
| Current-source unit regression | **Proved locally** | Through committed source `dbafb01b`, `/tmp/NoumSummaryFairnessFullFinal-20260714-2248.xcresult` records 4,102 unique `NoumTests` tests / 4,117 executions, zero failures, zero skips, and result `Passed` on the iPhone 17 simulator. The focused Summary-filler selection separately passes 19/19. This is a dirty-main-checkout unit run and does not supersede the clean signed full-scheme, Release-scan, UI, physical-device, deployed-backend, or external evidence gates. |
...
| Speech-quantity fairness across evaluation, goal evidence, and immediate prescription | **Proved locally** | At `c3d1ff65`, one pure `FillerBurden` projection drives severity, drill focus, cross-session trends, filler-aligned copy, and controlled pressure-stretch eligibility. At `ec10990f`, severe qualifying burden routes to Ah Counter (`Filler Control`), including after a first severe qualifying rep; non-severe thin evidence stays suppressed. Home/Train recommendation inputs and Summary's shared builder use qualifying per-session rates and exclude sub-15-second samples. At `4da56e3e`, Timed/Ah evaluation uses the same qualifying rate for score, XP, feedback, clarity, weak moments, insights, and recent-session comparison; 1/60 equals 10/600, 3/20 stays concentrated, and sub-floor speech withholds judgment. Sudden Death retains exact-zero tolerance. Immediate severe pace requires 15 seconds, 20 words, and finite WPM. At `ac393554`, the latest trajectory pack, qualitative goal rubric, goal-card confidence, and public weekly filler trend also require quantity-qualified, duration-normalized evidence; missing/tiny reps cannot demonstrate control or be promoted by mature history. At `5bbb850b`, the action path consumes its finalized trend input deterministically rather than rereading shared state. At `dbafb01b`, Summary presentation, Practice insights, `CoachingPlanner`, and chronological Review/Profile comparison consume the same qualified rate; historical movement requires two qualifying priors and current schema. The focused 19 tests and complete 4,102-unique-test / 4,117-execution unit target pass. The direct coach-context filler-question response and latent Path quality remain outside this proof. This proves local consistency, not professional calibration or user benefit. |
| Production readiness | **Missing** | The clean detached `70b0b380` readiness rerun remains NO-GO, 18/100, local target shape 85/100, maximum allowed 20/100, claim `localEvaluationSubstrateOnly`. Descendant commits through `dbafb01b` add local ineligible observation, friendship authority, bounded cross-account exact-PCM replay, goal-calibration review-intake integrity, dormant exact challenge provenance, a process-local exact-session route lease, checked-in Firebase backend deployment locks, duration-normalized filler prescription/evaluation/goal/Summary/planner evidence, deterministic finalized trend ownership, the literal severe-filler-to-Filler-Control route, and a qualified severe-pace shortcut, but earn no external readiness points. The current Functions source has a clean canonical local gate, not a new attachment-backed release capture. No validated attachment-backed release run is available, zero of five required external sidecars is present, and no current-source hosted privacy-body probe exists. The main checkout additionally preserves the user's unrelated localization edit and staged screenshot handoffs. |
