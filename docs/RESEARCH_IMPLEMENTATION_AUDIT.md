...
| Goal read → auto-prescribed drill | **Incomplete** | `NextActionEngine` owns the action, but it does not consume `GoalOutcomeRead.nextDimension`, movement, or its qualitative proof test. The explicitly chosen style currently affects rationale copy and a late/default drill tie-breaker; Summary computes the outcome read separately after the finalized action exists. That read currently flows through raw-count trajectory and reasoning inputs, so duration normalization is a correctness prerequisite to routing it. |
