...
| Fast first session | `FastLaneOnboardingView`, `CoachingProfileDraft`, the existing `CoachingProfileStore`, `FirstRunOnboardingGate`, and a structure-only `RoleplayEngine` projection deliver offline written first value without speech permissions or fake delivery evidence. The raw response is transient; only a bounded content-free receipt persists. Full setup is prefilled/resumable and can continue into the existing consent/Timed path. `AutoGuidedFirstRep` remains a separate default-off spoken path. | Implemented locally; spoken and structured evidence remain distinct | Signed-device elapsed-time, permission/consent, interruption/relaunch, and structured-to-live validation; experiment assignment still needs a privacy/product decision |
...
| Activation/transformation KPIs | Account-scoped `FlowEventLog`, `TransformationKPIReport`, session/provider history, recommendation outcomes, review/coach/notification lifecycle events, and the three-rep qualitative outcome question. Structured first value has its own completion/time fields; `firstRepCompleted` remains durable spoken-session truth. Structured-to-live upgrade is a paired content-free correlation read. Prescription acceptance is shown-to-tap; cloud fallback includes only cloud-requested routes resolved locally. | Implemented locally in M26 plus the fast-lane closure | Cohort aggregation requires an explicit privacy/telemetry decision; no transcript or third-party analytics SDK is used |
...
The local KPI report now covers first-value completion and elapsed time separately
from durable first-spoken-rep completion and elapsed time,
sessions per active week, review-open rate, shown-to-tap prescription acceptance,
structured-to-live and typed-coach-to-live upgrade, 7/28-day qualitative goal movement, notification
