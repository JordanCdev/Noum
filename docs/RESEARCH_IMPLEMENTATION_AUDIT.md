...
`eb597589eefa433f9cf2225341ac1cf563cb595f`
...
| Coach-arena contracts at the evidence baseline | **Proved** | 119 Node contracts and 136 Python runner contracts pass. The suite proves default-deny committed and uncommitted source classification, NUL-safe rename/copy/untracked handling, exact generated-output exclusions, no-overwrite sidecar refusal, and existing score/trace/readiness behavior. The canonical artifact remains bound to `80fbf6d2`; it is no longer current for `eb597589`. These are local tooling and evaluation contracts, not live-provider or human-outcome proof. |
...
| Production readiness | **Missing** | Gate result remains NO-GO, 18/100, local target shape 85/100, maximum allowed 20/100, claim `localEvaluationSubstrateOnly`. At `eb597589`, source freshness still blocks on `currentGitCommit` because the active dump is bound to `80fbf6d2`; the current primary checkout additionally blocks on `dirtyCoachSource` because the preserved localization edit is not part of a source-bound artifact. No validated attachment-backed release run is available to the gate, and zero of five external artifacts passes. |
...
as named historical baselines; neither is the current `eb597589` source
boundary. `9378b6ef` remains the focused recommendation sync boundary, while
`eb597589` is the current tap-attribution boundary; neither is a replacement
for the complete `47cbab5f` regression. Regenerate the local
