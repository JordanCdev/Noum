...
| `Noum/FirstRepCelebration.swift:431` — `loadProof` fires `withAnimation(.easeOut(duration: 0.45)) { self.proof = resolved; self.noticeFlashID += 1 }` unconditionally. Drives the proof-line fade-in (`.transition(.opacity.combined(with: .offset(y: 6)))` at line 219) when the verbatim quote lands. **Not gated on `reduceMotion`** even though the file's `runSequence()` correctly gates all other animation phases. | **NICE-TO-HAVE** (M15-introduced — Phase 2 b5f8d56) | One-shot fade-in, not a vestibular concern by itself — but the surrounding entrance choreography branches explicitly on `reduceMotion` (line 318 `guard !reduceMotion else { … resolve all phases instantly}`). The inconsistency means reduce-motion users get a sudden y-offset slide on the observation slot while the rest of the screen settled instantly. Suggested fix: wrap the animation in `withAnimation(reduceMotion ? nil : .easeOut(duration: 0.45))`. |
