...
gates. Later verifier commits `4dd2182b` and `518b3103` require the exact 11-
callable runtime roster and exact hosted privacy body; they do not change coach
output or earn external readiness. The hosted body currently differs from
source, so privacy deployment evidence remains missing.
...
| Trust and reliability | Local speech, consent routing, startup-fallback transparency, privacy, export/deletion, exact processor-manifest generation, private-profile validation, atomic evidence, default-deny source binding, legacy-route containment, source-bound Apple preflight, the 14-surface/77-check TestFlight contract, the exact 11-callable runtime-security inventory, and exact hosted-policy-body verification are implemented locally | Replace the destructive social migration with recoverable quarantine/resume/rollback mechanics and fault-injection tests. Then revoke the leaked legacy Deepgram credential and both exposed Firebase CLI sessions, deploy the checked-in privacy body, verify production IAM/App Check, inventory active clients and private-profile values, approve a minimum-client policy, protect every legacy route, audit provider usage/billing, install paid-team distribution authority, and complete physical-device and release-policy verification |
