...
Ask Noum failed-acceptance repair and production-evidence closure in progress
...
## Current production-closure slice — Ask Noum repair and bounded coaching-content recovery

The 2026-07-17 product verdict is explicit **NO-GO**. The reporter usually saw
Ask Noum as unavailable and rejected the replies that did land as repetitive,
unnatural, and unlike a human expert communication coach. That is a failed core
coaching loop, not a launch-ready implementation waiting only for formal live
evidence.

The current dirty source extends the existing chat transport, context builder,
reply pipeline, evidence gates, and deletion fence. `coachChatV2` is an additive
normal app route whose personal-evidence, general-coaching, memory-handoff, and
conversational response kinds isolate context and fallback behavior. General and
conversational replies receive no inherited personal assessment/history;
personal prescriptions require qualified same-dimension evidence; a no-move
brief makes zero provider calls; and generated coaching fails closed when it
cannot produce a grounded move. Supported general turns have a bounded intent-
specific recovery for typed quality rejection, while unsupported, empty, and
malformed output remains an explicit failure. The distinct schema-v1
`coachChat` source path
locally freezes the historical policy, validation, context, one-generation
stream, errors, and seven-field completion. That is local semantic-contract
proof only, not deployed production compatibility.

The immediate availability root cause is now verified rather than inferred. A
read-only production inventory exposes only `coachChat`,
`coachChatAvailability`, `deleteAccount`, and `transcriptionToken`; the app's
required `coachChatV2` endpoint is absent. The client now requires the
availability endpoint to advertise that exact function, request schema 2, and
the `noum-coach-v2` policy. A legacy `{available:true}` response becomes a
specific backend-version limitation rather than a generic network error. There
is deliberately no silent v1 fallback because production v1 has five
...
Personal provider input now contains the typed brief once and a small turn-
scoped style/craft context, not the same decision repeated through history,
plan, case, assessment, and prescription blocks. The server response ceilings
are restrained, and deep length requires an explicit deep ask. The server
prompt and deterministic fallback omit a stored drill when the user asks for an
explanation or judgement. The deterministic server gate now rejects bounded
imperative, modal, recommendation, and indirect-action shapes on those personal
turns even when the typed brief stores a continuity move; repair explicitly
removes the drill, and policy exhaustion uses the existing evidence-only typed
read. The iOS landing gate mirrors that conservative protection. Explicit move
requests, descriptive evidence, general craft, and memory handoff remain
available. This is lexical enforcement, not proof against every semantic
paraphrase. Deterministic fallback uses natural evidence-then-move wording
without forcing `because`.

The newest cross-turn repair closes an impossible local contract for a retained
intervention. The old prior-action gate rejected the typed current move when it
matched recent coach history, while changing it failed typed grounding; rewrite
exhaustion could therefore surface as unavailable. Server and iOS policy now
canonicalize controlled intervention synonyms, reject exact and paraphrased
drill restatements, and allow only an evidence-led “stay with that focus”
continuation when the user explicitly asks for action and the typed brief really
retains the recent move. Deterministic exhaustion uses that same concise shape,
repair prompts no longer force `because` or `so`, and clarification copy answers
the user's concern without repeating the drill. This remains lexical and
deterministic local evidence, not proof that generated replies feel expert.

Trust repair now carries
the challenged coach answer plus the complaint, elliptical general follow-ups
retain one bounded prior-user referent, and cold-start evidence guards cover the
general lane. Server repair exhaustion is typed separately from incomplete
generation. The service owns one remote capability preflight per attempted turn;
the view does not repeat it and the Firebase stream only revalidates local
identity, so typed backend and service reasons survive the request boundary.

The availability repair now contains one bounded source path for a
`local-guest-*` fallback. `AuthManager` retries a new Firebase anonymous
identity after hydration/foreground readiness and exposes an explicit Connect
action in Ask Noum and Settings. A three-phase, Keychain-backed journal makes
the local namespace authoritative before commit and the target permanently
authoritative afterward. `AccountDataRegistry` copies exact account-scoped
values, rejects target conflicts before writing, rebinds self-owned challenge
IDs, and excludes remote/derived bookkeeping, unattributed device state, and
recordings. Promoted anonymous guests cannot sign out and strand their only
credential.

Identity creation does not imply coaching-content upload. Current versioned
cloud consent gates profile, XP, session/transcript/evidence, and recommendation
writes. Outbound content work is bound to the lifecycle at scheduling, exact
account/provider, hydration phase, transition fences, current durable consent,
and matching Firebase UID; recommendation mutations additionally send the
expected account for server-side comparison with verified Auth. A failed
consent persistence/readback closes upload authority instead of reviving an
older durable allow. A separate target marker retries the idempotent snapshot
without blocking linking or deletion, and Ask Noum refreshes when exact
hydration lands.

Ordinary profile, progression, and session transport now shares one account-
scoped, metadata-only journal. Existing stores remain payload owners; transport
resolves their exact account namespace, drains serially, and clears only the
exact revision/mutation UUID that completed. Relaunch/hydration, foreground,
and consent allow resume pending work, while corrupt journal state fails closed.
The newly promoted empty target's snapshot uses the same lane and exact task
ownership; existing accounts no longer receive a blind whole-state consent
flush. Bootstrap preserves dirty profile/progression/session work, account
deletion drains in-flight content before remote destruction, and replacement
writes remove cleared optional fields.

This narrow local-to-new-anonymous route is not general account migration:
existing-account merge, complete cross-device restoration, the Apple/Google
link-success-before-Keychain crash window, and same-UID recovery after loss of
an anonymous Firebase session remain source gaps. Client ordering and relaunch
retry are now bounded locally, but there is no server CAS/mixed-client cutover,
startup digest repair for the local-persist/journal-enqueue crash window, XP
award-event provenance, or remote session tombstone. Current schema-2 session
writes were also rejected by the checked-in schema-1-only rule; source now
admits schemas 1 and 2 and rejects unknown schema 3, but deployment is unproved.

Current local backend verification passes TypeScript lint/build, **161/161**
Functions tests, **7/7** deploy-lock tests, and **19/19** cloud-operations
validator tests. Coach Arena's **9/9** simulator-environment checks reduce
accidental shared-app replacement but do not prove simulator ownership or
disposability.

Current-source iOS verification passes **124/124** typed-evidence routing,
provenance, policy, secure-wire, and pipeline tests in
`/private/tmp/NoumTypedEvidenceFinal8-20260717.xcresult` plus **133/133** reply-
...
the final typed projection and natural-stat-routing edits. These are
deterministic local results, not deployed wording acceptance. The earlier
**30/30** Auth/Firestore/Functions emulator result, signed **224/224** Ask
bundle, normally signed Debug and unsigned Release builds, and six-frame sweep
also remain historical. A post-edit unsigned Debug simulator build succeeds;
current emulator, signed Debug, unsigned Release, and visual evidence have not
been refreshed. Earlier promotion/privacy/authority **41/41**, journal/
promotion/registry **22/22**, and deletion-admission **4/4** results remain
bounded historical evidence for those assertions.

The direct-evaluation seam and bounded typed projections are now implemented
locally. Natural latest-stat wording, including contractions and polite
requests, selects grounded personal evidence while how-to/benchmark questions
remain general coaching. Latest metrics use canonical persisted word/WPM and
qualified filler provenance. Longitudinal reads require two to five unique
...
- select the final source in a signed Ask build and rerun the current
  Auth/Firestore/Functions emulator contract, unsigned Release build, and
  affected visual/availability flows;
...
- capture current-source App Check-valid generated conversations for the
  representative five-case set, then obtain reporter acceptance and independent
  professional-coach review of the visible wording; and
- exercise the narrow local-to-new-anonymous path on physical/TestFlight
  hardware, including timeout, retry, consent decisions, relaunch at every
  phase, deletion, and later Apple/Google linking.

After the Ask core loop passes those boundaries, continue the separate content-
sync hardening track:

- add server-mediated expected-revision/CAS semantics and an explicit mixed-
  client minimum-version cutover for ordinary profile, progression, and session
  documents;
- add startup digest reconciliation for the payload-persisted/journal-not-yet-
  enqueued crash window;
- replace aggregate-only XP synchronization with award-event provenance and add
  remote session-deletion tombstones;
- prove two-build/two-device convergence, optional-field clearing, relaunch
  retry, and dirty-bootstrap preservation;
- deploy and read back the schema-2 Firestore rule before treating current
  session upload as operational; and
- design and prove existing-account merge, complete remote restoration, and the
  Apple/Google link-success-before-Keychain recovery window.

None of those deployment, mixed-client, identity, or generated-quality gates is
complete. The latest authoritative source-bound result remains the inherited
`ac664112` **NO-GO at 18/100 with 0/5 required external artifacts**, not a fresh
readiness run for this dirty source.
...
Forward Plan account/source/authorization lease with the product-wide metric
qualification boundary. Provider-visible recent filler context now uses the
shared historical filler projection and emits only a qualified per-minute rate
or `not measured`. Persisted filler and pace baselines use the current-schema
comparison accessors; stale or insufficient comparison aggregates are absent,
while independent score context remains available. No new metric owner, plan
store, or routing path was introduced.

Focused verification passes **7/7**, the combined metric and Forward Plan
selection passes **134/134**, and the complete unsigned target passes **4,355
unique tests / 4,374 device-configuration executions** with zero failures or
skips. Its result bundle is
...
- surface one coherent first-plan entry in a primary path. The existing entry
  is source-reachable after three eligible reps through Profile → Library →
  Coaching evidence → Coaching direction, but it is deeply buried; Home
  suppresses `.prompt`, and Ask Noum has no dedicated entry;
- explain a legitimate fail-closed generation result instead of navigating to
  an unchanged Ask thread;
- close Forward Plan admission at deletion initiation, cancel owned provider
  work, and persist the deletion fence across relaunch;
- bind AI-call diagnostics to account lifecycle and registry deletion, reject
  delayed cross-account writes, purge unattributable legacy rows, and remove
  the optional provider-draft persistence path; and
...
local teardown or caller cancellation, and an in-process plan/thread commit can
still occur before teardown. Successful teardown later removes account-scoped
writes; only completions after teardown fail closed. AI-call diagnostics also
remain device-global metadata outside the account registry. Their reason field
can retain interaction and coaching-gate details,
and an opt-in live-evaluation flag can append provider draft fragments even
though no checked-in build setting enables it. The two local persistence writes
are not crash-atomic, and no Forward-Plan-specific rendered, physical-device,
live-provider, professional, longitudinal, operational, or
...
practice focus, and an unavailable transition completes without claiming a
specific next rung or objection. The focused Roleplay selection passes 38/38,
the deterministic rendered lane passes 3/3 including Accessibility XXXL, and
the complete target passes 4,188 unique tests / 4,205 device executions. This
closes the previously named bounded local candidate but earns no external
readiness point. Wider Roleplay prescription routing and durable turn-duration
evidence remain product/schema decisions; all five required external artifacts
remain missing, so production readiness remains NO-GO at 18/100.
...
mixed invalid-history branch. The later terminal Roleplay closure is recorded
above; all five required external artifacts remain missing, so production
readiness remains NO-GO at 18/100.
...
The later Review projection and terminal Roleplay closures are recorded above.
All five required external artifacts remain missing, so production readiness
remains NO-GO at 18/100.
...
unlocks before append, then resolves the exact newly unlocked node and triggering
session after dependent state lands. General Path criteria reject sub-three-word,
sub-three-second, non-finite, and evaluation-only rows; the stricter filler
evidence floor and durable grandfathered unlock ledger remain intact. Focused
and related selections pass 23 and 55 unique tests respectively, and the full
target passes 4,145 unique tests / 4,162 device executions. A light five-tab
sweep is visually clean but does not prove the earned-landmark overlay. This is
local believable-progress integrity, not retention, transfer, physical-device,
or launch evidence.

The explicit-style, rendered-Home ownership, unified four-mode availability,
Train/Prep projection, active-week phrase handoff, typed-ID Speech Project
...
Later readiness hardening at `24b5ad6a` makes the uncommitted-source boundary
repository-wide and default-deny. Any staged, unstaged, renamed, copied, or
untracked behavior/resource/project/test/evaluator path blocks sidecar creation,
app-path freshness, readiness, and release-evidence binding even when the manual
coach fingerprint matches. Only approved documentation and eight exact
workflow-owned app-path outputs are excluded. This tooling-only boundary passes
119 Node, 136 Python runner, 29 release-evidence, and 31 TestFlight-preflight
tests; it does not replace the `47cbab5f` Swift regression.

The current clean implementation boundary `7ae1fe43` passed the full signed
simulator regression with 4,061 tests, zero failures, and zero skips, plus the
optimized Release build and bundle scan. The latest canonical Coach Arena
capture at `70b0b380` contains 50 required fixtures / 109 traces, scores 79.76,
and passes local app-path, real-pipeline, production-evidence, and trace-quality
gates. Later verifier commits `4dd2182b`, `518b3103`, and `81c17f4c` require the
exact 13-callable runtime roster and exact hosted privacy body; they do not earn
external readiness. The hosted body currently differs from source, so privacy
deployment evidence remains missing.

Goal-calibration intake hardening at `a5d5c365` makes the existing professional
review gate reject empty or partial coverage, non-independent or wrong-role
reviewers, malformed rubric dimensions, out-of-contract risk/notes, and access
receipts attributed to different reviewers. Its focused 14-test suite and the
12-case artifact dump pass locally. This is packet-integrity evidence only: no
access-controlled evidence package or professional result exists, so it earns
no readiness points and authorizes no numeric product surface.

Challenge-provenance hardening at `23fbfeee` carries the exact server-created
prompt and existing content-free observation intent through the established
opaque Timed handoff without normalizing bytes or creating another route owner.
Account, token, prompt, Unicode, and prompt-replacement checks fail closed; the
armed-rep guard is byte-exact. At `c7cd0b51`, that arm is a process-local lease
created only after exact handoff consumption and authoritative cached
participant/expiry/unplayed validation, then bound to the exact saved session
before Summary. Reload/export/migration no longer preserve prompt-bearing arm
state, and matching cancellation, retry/terminal cleanup, account teardown, and
legacy-key purge fail closed. A focused 73-test simulator selection passes.
Both capabilities remain false, the challenge sheets still lack a production
call site, and no eligible evidence writer exists, so this earns no readiness
point.

Recommendation-response hardening at `f1d6ec72` replaces global/raw historical
averages with a bounded persisted-demand comparator, filler-per-minute evidence,
metric-specific pace/filler/general interpretation, unique-session floors, and
session/outcome schema provenance. A detached clean-boundary simulator run
passes 75 focused tests with zero failures or skips. It is focused local
evidence and likewise does not replace the complete `47cbab5f` regression or
earn any external readiness row.

Immediate prescription now uses the same duration-normalized filler model. At
`c3d1ff65`, the established action, drill, and trend owners interpret qualifying
filler evidence at 2/3/5/8 per minute, require a 15-second sample, and require at
least three detected fillers for the severe tier. This prevents long low-rate
Speech Project reps from being escalated on count alone while still isolating a
short qualifying high-rate rep. The rebuilt complete unit target passes 4,061
...
At `c44bd91c`, Mini-drills and Lesson Apply join the shared recording lifecycle
contract. Their clocks remain stopped until capture readiness; evaluation and
progress require a usable terminal transcript receipt. Failed setup,
interruption, silence, or finalization therefore cannot manufacture a
low-filler mini-drill success, XP/history, or lesson progress. Both surfaces
reuse the bounded recovery copy and content-free startup-fallback notice. The
focused 13-test integrity selection passes. This closes a local false-progress
path only: mid-stream provider replay remains intentionally unsupported, and
real on-device model, interruption, route-change, and live-provider behavior
still require physical-device/TestFlight evidence.

At `4da56e3e`, the same speech-quantity policy reaches the score-facing
evaluation path. Timed Practice and Ah Counter now use qualifying
fillers-per-minute for score, XP, feedback, category ratings, weak moments,
insights, and recent-session comparisons, so equivalent rates receive
equivalent reads across ordinary reps and long Speech Projects. Sub-15-second
speech withholds filler judgment, explicit zero evidence remains meaningful,
and Sudden Death retains exact-zero pressure semantics. Immediate severe pace
also requires the shared 15-second / 20-word floor and finite WPM. The focused
selection passes 53 unique tests and the complete simulator unit target passes
4,075 unique tests / 4,090 executions with zero failures or skips. The next
coherence gap is upstream: `UserTrajectoryCache`, `TrajectorySummaryBuilder`,
and `CoachReasoningPass` still carry raw filler counts into the qualitative goal
read, so that read must become duration-fair before it can drive the existing
action owner.

At `ac393554`, that policy now reaches the qualitative goal and trajectory
evidence boundary. `UserTrajectoryCache` withholds WPM below 15 seconds / 20
words and emits a recent filler-rate trend only when all three reps qualify.
`CoachReasoningPass` treats missing or undersized mechanics as missing evidence,
uses qualified fillers per minute for hedge and pacing deductions, and no longer
turns a tiny filler count into a prescription. `TrajectorySummaryBuilder`
requires two qualifying reps per week for its public filler-rate signal, and
`GoalOutcomeEngine` prevents mature history from promoting a sub-floor latest
rep. At `5bbb850b`, the existing drill engine consumes the finalized
`NextActionInput.trends` instead of rereading mutable shared trend state, making
the action decision deterministic under the complete suite. The affected 57
tests pass, followed by 4,083 unique unit tests / 4,098 executions with zero
failures or skips. This closes the correctness prerequisite inside the goal
read. At `dbafb01b`, Summary, Practice insights, `CoachingPlanner`, and
chronological Review/Profile comparisons consume one quantity-qualified filler
rate presentation boundary. Current evidence requires 15 seconds, 20 words, and
adequate transcript confidence; historical comparisons require current schema,
...
tests now consume the same exact qualified projection. Qualified evidence cites
count, duration, and fillers per minute; thin or uncertain evidence withholds the
comparison and gathers a comparable rep. Unsupported count scraping, close
cluster certainty, causal “pressure leak” language, and raw-count success checks
are removed. The complete unit target passes 4,109/4,109.

At `a2502c04`, the persistent Clean rep and Filler-free week Path milestones join
that shared quantity, confidence, schema, and fixture boundary. Existing unlock
IDs remain durable through the unchanged JSON-array ledger, while new progress
must satisfy the corrected rule. Path copy now discloses the 20-word / 15-second
...
At `c3e018d4`, Roleplay stops applying its duplicate raw word-ratio filler
heuristic. Because Roleplay turns do not retain finalized duration, the engine
now withholds filler evidence from quality and retry-pressure selection rather
than contradicting the shared quantity-qualified fillers-per-minute contract.
Semantic uses of “like”, “actually”, “kind”, and “sort” stay neutral while the
existing directness, evidence, listening, question, and explicit-hedge signals
continue to operate. Pace Training, Cut the Crutch, and Roleplay also reuse the
shared content-free `transcriptionRouteNotice`, completing source-level wiring
across all nine speech surfaces. Focused fallback verification passes 26 unique
tests, focused Roleplay verification passes 24, and the full unit target passes
4,139 unique tests / 4,156 device executions with zero failures or skips. The
light simulator sweep rendered all five tab tops, but it did not force a real
provider failure; physical/TestFlight fallback presentation remains external
evidence.
...
fixtures pass, average 79.78, with real-pipeline and trace-quality gates green.
...
These local commits do not change the authoritative release result: **NO-GO at
18/100 with 0/5 external artifacts**.
...
The longitudinal gate uses `coach-real-user-transfer-outcomes-v4.json`. Its
...
perfect sample: at least 28 elapsed days, 30 qualified qualitative participants,
200 qualified App Store Connect installs with D1/D7-eligible denominators, 60%
positive transfer, and 70% non-regression across the full qualifying set. Coherent
enrollment/withdrawal/exclusion accounting, retained negative outcomes, population
provenance, and evidence-backed resolution for every adverse outcome are required.
The separately reported 500-install conversion-scale signal is nonblocking for
first release.
