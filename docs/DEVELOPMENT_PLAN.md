...
| Actionable coaching | Rewrite, semantic-preservation guard, a Phrase Bank→active-week→Timed loop, one finalizer-owned Summary prescription, rendered-surface-owned Home exposure/tap measurement, Train's single coherent presentation snapshot, and one fail-closed four-mode availability projection across Summary, Home, Train, Ask Noum, and Prep are implemented; followed-rep learning requires explicit acceptance plus matching mode, and response learning now uses bounded unique recent sessions, persisted-demand matching, filler-per-minute normalization, metric-specific interpretation, and versioned session/outcome provenance; unavailable Prep pressure/audience steps receive category-bounded Timed prompts without changing readiness identity; Speech Projects retain catalog identity, curated prompts, and project timing through the real Timed route, but remain separate from adaptive prescriptions; the unread global prescription snapshot was removed; an exact-token generic-review versus outcome-loop assignment/exposure/next-rep contract exists but is inactive | Move Home/Train's synchronous shown write ahead of tap-time capability fallback so shown-only fallback remains in the denominator. Then persist Timed/Sudden Death difficulty and Speech Project identity if product owners approve those as longitudinal demand, align `PrimaryFocusMemory.followedRepValues` with accepted-prescription and normalized-filler evidence, and harden whole-ledger write serialization. After those local repairs, obtain product/privacy approval, configured allocation, population analysis, live-provider acceptance, and physical-device proof before expanding the prescription domain |
| Trust and reliability | Local speech, consent routing, startup-fallback transparency, privacy, export/deletion, a process-local account- and route-bound Timed prompt handoff, a versioned single-source processor manifest with readiness-enforced generation freshness, Codable-aligned private-profile Firestore validation, atomic live evidence, exact launch-prerequisite/history-scan validation, documentation-only clean-ancestor evidence reuse, repository-wide dirty behavior-source rejection with exact generated-output exclusions, a status-only five-route containment probe, source-bound Apple archive preflight, and the exact 14-surface/77-check TestFlight evidence contract are implemented | Refresh the now-stale canonical local coach artifact from a clean current source; the preserved localization edit correctly prevents using this checkout for that refresh. Then revoke the leaked legacy Deepgram credential and both exposed Firebase CLI sessions, inventory/migrate any unknown private-profile enum values before rules deployment, protect/disable every legacy route, audit provider usage/billing, install paid-team distribution authority, and complete physical-device and release-policy verification |
