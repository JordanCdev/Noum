...
| EXERCISE-5: Structured-argument frameworks (PREP; persuasion claim/counter; AREA) | shipped-partial | PREP exists; `FrameworkDrillChecks.claimCounter` and `FrameworkDrillChecks.areaAnswer` are routed through `structure.claimCounter` / `depth.areaAnswer` with copy + XP-invariance tests. Monroe's Sequence and a dedicated guided CEW drill remain unbuilt |
...
- **EXERCISE-5 (remainder)** — Monroe's Sequence and a dedicated guided claim-evidence-warrant drill. PREP, claim/counter, AREA, and the shared argument-logic read already exist.
