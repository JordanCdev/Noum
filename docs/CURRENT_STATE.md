...
- **URL scheme:** `noum://` registered in `Info.plist`. `DeepLinkRouter`
  ...
  `noum://ask`/`asknoum`, `noum://growth`/`library`,
...
- `Noum/ProofMomentArchive.swift` — per-account `ProofMomentStore`
  ...
  into the context block). Future Profile "growth library"
  surface will read from the same store.
...
- **M15 closed + M16 retention surfaces shipped (2026-05-21)** —
  ...
  - **M16 Growth Library (closes VISION pillar #4 "Believable progress")** —
    new `Noum/GrowthLibraryView.swift` (synced folder, auto-discovered
    by the Xcode `PBXFileSystemSynchronizedRootGroup` target). Persistent
    on-demand surface listing every banked `ProofMomentRecord` in
    `ProofMomentStore.shared` as a card stack (most-recent-first via
    `recent(limit: maxStoredRecords)`). Each card mirrors the existing
    `AIWeeklyInsightCard.proofSection(...)` visual register — brand-purple
    quote-icon eyebrow with a relative-date label, brand-purple technique
    chip, italic verbatim quote, secondary claim line. Reads-only — no
    new state, no migration, no AI calls, no counters. Closes the
    archive's ephemeral-only-surface gap: today proofs surface one-at-a-
    time on `AIWeeklyInsightCard` (proof of the week), `PathNodeCelebration`
    (on unlock), and `PersonalBestCelebrationScreen`, plus the Ask Noum
    context block — but the user has no place to *re-read* what the coach
    noticed. Empty-state branch renders an honest "Library waiting" card
    (only reachable via the race where the user wipes the archive after
    entering the screen; the Profile + Ask Noum entries gate on
    `count > 0` so the cold-start path never lands here). New
    `AppDestination.growthLibrary` case wired through `ContentView`'s
    `.navigationDestination(for:)` block + a `noum://growth` /
    `noum://library` deep-link route (append-only, doesn't reset the
    path — Profile users keep their context). Two existing surfaces gain
    affordances into the library:
    - **`ProfileView.insightsBankedChip`** — chip is now a `Button`
      whose label keeps the count + recency text but appends a quiet
      chevron and opens `noum://growth` via `@Environment(\.openURL)`
      (same DeepLinkRouter handoff pattern as `askNoumProfileLink`).
      Accessibility ID `profile.growthLibrary.link`, hint "Opens the
      growth library." Visually still ambient.
    - **`AskNoumView.insightsCaption`** — caption above the input bar
      gains the same chevron + tap target + `noum://growth` open.
      Accessibility ID `askNoum.growthLibrary.link`. Required adding
      `@Environment(\.openURL)` to AskNoumView (wasn't previously
      consumed).
    Per-row date label is a static pure function `relativeDateLabel(_:now:)`
    with proper singular / plural agreement across day / week / month
    buckets (a single-month-old proof reads "1 month ago", not
    "1 months ago"). VoiceOver label is a sibling static
    `accessibilityLabel(for:)` that flattens date + technique + quote +
    claim into one coherent sentence, omits the claim slot cleanly when
    the deterministic-fallback path produces an empty claim. Eleven new
    `GrowthLibraryTests` lock both helpers: relative-label boundaries
    (Today / 1 day ago / 2–6 days / 1 week / N weeks / 1 month / N
    months including the singular-month fix), accessibility slot
    order (date → technique → quote → claim), empty-claim suppression,
    and the "You said:" attribution that prevents VoiceOver from
    misreading the quote as the coach speaking.
