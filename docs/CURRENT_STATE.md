...
_Last updated: 2026-05-25 (M24 deferred slate round 10 — tone-drill reaches the practice picker + the post-session read: closes the top remaining "Future move" (#1) from the round-9 HANDOFF. The per-scenario tone-drill recommendation (round 9) was live only on the two Home surfaces; the two surfaces that ALSO call `RecommendationBiasEngine.blueprint` were still passing NO signal, and `PracticeModeSelectionView.appDestination(for: .imConversation)` launched with `scenario: nil, tone: nil` even when the blueprint carried them — so the drill was offered on Home but not where the user actually picks a mode or finishes a rep. This round wires both. `PracticeModeSelectionView.computeRecommendation` now passes `imToneSignal: IMModeAvailability.isAvailable ? IMHistorySummary.toneDrillSignal(from: sessions) : nil` and caches `blueprint.recommendedScenario`/`recommendedTone` into two new `@State`s; `appDestination(for: .imConversation)` returns `.imPractice(scenario: cachedRecommendedScenario, tone: cachedRecommendedTone)`, so tapping the IM tile when the engine surfaced a sub-40% tone-match scenario drops the user straight into that exact scenario + tone — and stays a default (un-prefilled) rep otherwise, because the engine leaves BOTH fields nil for every non-IM recommendation, so a non-IM bias can never leak a stale scenario into the IM tile. `SummaryView.summaryRecommendation` passes the same availability-guarded signal, so the post-session "Looking ahead" card now names the exact tone-drill in the user's own observed numbers after a non-IM rep, routing them to the now-wired picker. Purely additive at the engine: the `blueprint` signature/defaults are unchanged, so the four prior callers behave identically. New `RecommendationIMDestinationPrefillTests` (2 cases) locks the destination-prefill invariant on the profile path — an IM-biased profile (calmer-delivery + work + warm) populates both scenario (`.difficultConversation`) and tone (`.warm`); a non-IM bias (reduce-fillers) leaves both nil. Build-host has no Swift toolchain, so nothing was compiled or run; the change reuses the round-9 call-site pattern verbatim and still wants `xcodebuild test` plus a device glance at the picker's IM tile and the Looking-ahead card for an account with a sub-40% tone-match scenario.)_
