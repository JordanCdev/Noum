...
- **Not run:** the live harness itself (needs provider API keys — compile-verified only) and the full 1900+ test suite (time-boxed). The change is additive + flag-guarded; a full `xcodebuild test` pass is the recommended pre-TestFlight gate.
