...
//
// Apple's `Font.custom(_:size:)` resolves variable fonts by family name and
// then a weight is applied with `.weight(_)`. SF Pro Rounded / SF Pro Text
// remain the fallbacks if Figtree/Manrope fail to load (e.g. preview targets).
...
enum Typography {
    ...
    static let display = Typography.figtree(size: 42, weight: .bold)
    /// 40 / bold / rounded — login hero
    static let hero = Typography.figtree(size: 40, weight: .bold)
    /// 30 / bold / rounded — screen titles ("Choose your next rep")
    static let screenTitle = Typography.figtree(size: 30, weight: .bold)
    /// 26 / semibold / rounded — section heroes ("Hello, Jordan")
    static let sectionHero = Typography.figtree(size: 26, weight: .semibold)
    /// 28 / bold / rounded — big stat values ("Speaker 1", rating numbers)
    static let bigStat = Typography.figtree(size: 28, weight: .bold)
    /// 20 / bold / rounded — card titles
    static let cardTitle = Typography.figtree(size: 20, weight: .bold)
    /// 18 / bold / rounded — recommendation/headline copy
    static let headline = Typography.figtree(size: 18, weight: .bold)
    /// 16 / medium / text — login subcopy
    static let subheadline = Typography.manrope(size: 16, weight: .medium)
    /// 15 / regular / text — body / mode descriptions
    static let body = Typography.manrope(size: 15, weight: .regular)
    /// 13 / semibold / text — labels, targets, badges
    static let caption = Typography.manrope(size: 13, weight: .semibold)
    /// 10 / bold / rounded — uppercase micro-labels (tracking 0.8)
    static let micro = Typography.figtree(size: 10, weight: .bold)
    /// 10 / semibold / rounded — nav labels (TRAIN, REVIEW)
    static let nav = Typography.figtree(size: 10, weight: .semibold)

    // MARK: - Builders

    /// Figtree at the requested size + weight, with system rounded fallback.
    static func figtree(size: CGFloat, weight: Font.Weight) -> Font {
        Font.custom(Family.display, size: size).weight(weight)
    }

    /// Manrope at the requested size + weight, with system text fallback.
    static func manrope(size: CGFloat, weight: Font.Weight) -> Font {
        Font.custom(Family.text, size: size).weight(weight)
    }
