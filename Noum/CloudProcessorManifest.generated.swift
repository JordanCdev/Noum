...
enum CloudProcessorManifest {
    ...
    static let processors: [CloudProcessorDisclosure] = [
        ...
        CloudProcessorDisclosure(
            ...
            data: "Bounded production coaching requests, Authentication and App Check proof, and—only after current cloud-processing consent—coaching profile, XP, practice-session content including transcripts and session evidence, and recommendation state for account sync; bounded competitive-rep PCM held in invocation memory with server-derived observation metadata and an account-unlinked seven-day exact-audio replay digest, and short-lived mutual-connection invite secrets handled in invocation memory with digest-addressed account, display-name, pair, and timestamp records; account identity, Firebase installation data, and versioned Remote Config values are processed separately for account and configuration functionality",
            purpose: "Protected callable transport and abuse protection for cloud coaching, consent-gated account-content sync, competitive observation, and mutual account connections, plus authentication, first-run configuration, and vendor-declared diagnostics",
