...
Product analytics is off until you opt in under **Settings > Privacy**. If you opt in, Noum sends one bounded total for each closed UTC day through a Firebase Authentication and App Check protected function. The total can include counts for app activation, onboarding, practice, summaries, weekly reads, notification categories, paywall and subscription outcomes, coarse plan or trial state, app/build version, a coarse time-to-first-value bucket, estimated AI usage cost in US dollars, and a count of provider calls whose usage units were unavailable. It never contains microphone audio, transcripts, typed responses, prompts, generated coaching, goals, names, event identifiers, device identifiers, or an account identifier in the aggregate payload.

Firebase uses the current authenticated account and a server-only short-lived rate-limit record to verify and protect the request. Noum then combines the bounded counts into a day/build aggregate. A random retry identifier prevents the same daily total being counted twice; it is not derived from an account, device, installation, session, or event. You can withdraw product-analytics permission at any time. Withdrawal stops new uploads and removes an unsent retry envelope; already-combined counters cannot be linked back to you or individually removed. The account-scoped consent, upload cursor, and any unsent retry envelope are included in Noum's local export/deletion registry.
...
- **Optional product-analytics data** is uploaded only after opt-in. Anonymous daily/build counters are retained for up to 24 months for launch, retention, subscription, and cost analysis. Random idempotency markers carry a 35-day expiry timestamp; automatic cleanup depends on the corresponding Firestore TTL policy being configured and verified. Because combined counters contain no account identifier, they cannot be attributed to or individually removed for one account
