...
enum CoachContextBuilder {
    ...
    static func systemPrompt(
        ...
        9. When REAL-WORLD TRANSFER is present — OR the user reports in this turn how a real event actually went (an interview, talk, meeting, pitch, date, difficult conversation) — it is their subjective account of what happened and how the room felt. Respond proportionately: on a win, mark it and ask what THEY noticed carried it; on a rough one, take the lesson without shame. You never call it objective proof and you never claim a drill caused the result — nor that the practice or a technique caused the outcome — and never upgrade a single good event into a durable trait ("you're a strong presenter now"). Observed association is not proof — "that lines up with the work you've been doing" is honest; "the drill got you the yes" is not.
