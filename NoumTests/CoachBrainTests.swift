...
// MARK: - Softened observable-anchor gate
//
// Regression coverage for the one live gate fix: a grounded SOFT anchor
// (temporal marker + second-person action) now passes, while genuinely generic
// coaching still fails. The change only ADMITS replies, so existing rejections
// are unaffected.

@Suite("ObservableAnchorGateTests")
struct ObservableAnchorGateTests {

    @Test func softAnchorReplyNowPasses() {
        let reply = "Earlier you rushed the open, so hold a beat before your first line next time."
        let issue = AICoachChatService.replyQualityIssue(
            in: reply,
            latestUserTurn: "how do i get better"
        )
        #expect(issue == nil)
    }

    @Test func genericUnanchoredReplyStillFails() {
        let reply = "Try to be more confident and keep working on it."
        let issue = AICoachChatService.replyQualityIssue(
            in: reply,
            latestUserTurn: "how do i get better"
        )
    ...
    @Test func temporalAloneIsNotEnough() {
        // A time marker with no specific second-person action is still generic.
        let reply = "Earlier things were fine, just keep it up and stay positive."
        let issue = AICoachChatService.replyQualityIssue(
            in: reply,
            latestUserTurn: "how do i get better"
        )
