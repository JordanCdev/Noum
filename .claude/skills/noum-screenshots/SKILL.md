...
The current mode lives at `.claude/skills/noum-screenshots/.mode` — a single-line file containing `off`, `light`, or `detailed`.
...
1. **Read mode** from `.claude/skills/noum-screenshots/.mode`. If `off`, skip to step 6 (write minimal HANDOFF only). Otherwise continue.
